...
后端已支持通过环境变量接入 DeepSeek 审核服务：

```powershell
$env:AUDIT_PROVIDER="deepseek"
...
说明：
- 配置 `DEEPSEEK_API_KEY` 后，审核优先走 DeepSeek
- 如果 DeepSeek 调用失败且允许兜底，会自动降级到 mock
- 无论走 DeepSeek 还是 mock，接口返回 JSON 结构保持一致
...
- 日志记录：记录 DeepSeek 成功、失败、重试、降级情况
